...
function parseJsonCard(bytesIn) {
  ...
  if (!range) throw new Error('no character card JSON found');
  const card = JSON.parse(strFromU8(b.subarray(range[0], range[1] + 1)));
  ...
  return { spec: card.spec, specVersion: card.spec_version, name: data.name, assets, card, embeddedUnresolved };
}
