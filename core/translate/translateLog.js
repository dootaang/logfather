...
async function translateCombined(src, translateFn, opts, skipKorean, threshold) {
  ...
  for (let i = 0; i < src.length; i++) {
    const text = String(src[i] == null ? '' : src[i]);
    const { masked, tokens } = maskMarkup(text);
    if (!text.trim() || !hasProse(masked)) { out[i] = src[i]; tick(); continue; }
    if (skipKorean && isKoreanDominant(stripPlaceholders(masked), threshold)) { out[i] = src[i]; skipped++; tick(); continue; }
