...
const SECTIONS = [
  ...
  { id: 'library', icon: 'book', title: '서재', items: [
    ...
    '오늘의 서재 · 랜덤 뽑기: 홈의 주사위 카드를 누르면 서재에서 무작위 한 화가 열려요. 범위는 "안 읽은 화 먼저 / 전체 랜덤 / 즐겨찾기에서" 중 선택(기억됨).',
    '내 기록: 오늘의 서재의 "내 기록" 카드 → 내 레벨(불꽃 칭호 — 서재가 클수록 불이 커져요)·읽기 추이·월별 보관·최다 읽은 작품, 그리고 월/연 결산을 아카에 붙여넣는 카드로 만들어줘요.',
    '오늘의 명대사 · 그날의 로그: 서재 속 대사 하나가 매일 하나 뽑혀요(누르면 그 화로). 1년 전/한 달 전 오늘 만든 화가 있으면 "그날의 로그" 재회 카드도 떠요.',
    '검색·정렬: 작품·제목·본문으로 검색.',
    '도구 메뉴: 전체 내보내기 · 가져오기 · 전체 굳히기(데스크탑)는 홈 상단 "도구" 버튼 안에 있어요.',
  ...
  { id: 'reader', icon: 'bookOpen', title: '리더', items: [
    ...
    '페이지 이동: 페이지 넘김에서 하단 "12 / 120" 표시를 누르면 슬라이더(드래그=바로 넘김)와 입력칸이 떠요. 번호(50) 또는 비율(50%)을 치고 Enter. 키보드 Home/End = 처음/끝. 총 페이지 수는 글자 크기·창 크기에 따라 달라져요.',
    '읽던 자리 기억: 화마다 읽던 위치(페이지·스크롤)를 이 기기에 기억해 다시 열면 그 자리부터. 원문/번역·정리/원본 전환 때도 자리가 유지돼요.',
    '장 목차: 웹소설형에서 장 제목(편집기의 장 블록 제목)이 있으면 리더 좌하단에 "목차" 버튼이 떠요. 누르면 장 목록 — 현재 장이 강조되고, 고르면 바로 그 장으로(페이지 넘김·세로 스크롤 모두).',
    '책갈피: 리더 우상단 귀퉁이의 책갈피 아이콘(또는 B 키)을 누르면 지금 보는 페이지에 표시가 남아요(다시 누르면 빼기). 좌하단 "책갈피 N"에서 목록 — 그 페이지 첫 문장과 위치가 보이고, 고르면 바로 이동, ✕로 지우기. 로그인하면 기기 간 동기화, 원본 로그는 건드리지 않아요.',
    '형광펜: 본문 문장을 드래그하면 뜨는 팝오버에서 색(노랑·초록·분홍)을 고르면 그 문장에 칠해져요. 칠한 곳을 누르면 색 바꾸기·복사·지우기. 좌하단 "형광펜 N"에서 칠한 문장 목록 — 고르면 그 자리로. 정리/원본·원문/번역을 바꿔도 문장이 남아 있으면 그대로 칠해지고(번역 화면에서 칠한 건 번역 화면에서), 못 찾으면 목록에만 흐리게 남아요. 원본 로그·복사물·공유엔 형광펜이 안 들어가요(비파괴).',
    '표시 모아보기: 서재 작품 페이지의 화 목록에 책갈피·형광펜 개수 배지가 붙고, 내 기록(#/stats)엔 합계·표시한 화 수·최다 형광펜 작품이 나와요.',
    '공유 링크로 볼 때도 책갈피·형광펜을 쓸 수 있어요 — 다만 이 기기(브라우저)에만 남고 계정 동기화는 안 돼요.',
    '결산 카드엔 그 기간에 칠한 형광펜 중 최근 3줄이 "밑줄 친 문장"으로 들어가요(형광펜 색 그대로, 작품명·N화). 형광펜이 없으면 그 구역은 빠져요.',
  ] },
  ...
  { id: 'sync', icon: 'cloud', title: '동기화 · 계정', items: [
    '로그인 = 동기화 / 비로그인 = 이 기기에만 저장(로컬). 아이디 + 비밀번호로 로그인(이메일은 선택 — 비밀번호 분실 복구용).',
