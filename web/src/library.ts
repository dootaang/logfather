...
function importChatLog() {
  if (!chatInput) {
    ...
    chatInput.addEventListener('change', async () => {
      ...
      let parsed: any;
      try { const _obj = JSON.parse(await f.text()); parsed = parseRisuLog(_obj, f.name); if (_obj && _obj.assets && typeof _obj.assets === 'object' && !Array.isArray(_obj.assets)) parsed.assets = _obj.assets; if (_obj && Array.isArray(_obj.cleanupRegex)) parsed.cleanupRegex = _obj.cleanupRegex; if (_obj && Array.isArray(_obj.displayRules)) parsed.displayRules = _obj.displayRules; }   // ★다운로드 JSON의 assets 맵 + cleanupRegex(정리 정규식 동봉)
...
async function buildAndSaveChat(parsed: any, opts: any) {
  ...
  const displayRulesArr: any = (parsed && Array.isArray(parsed.displayRules) && parsed.displayRules.length) ? parsed.displayRules : null;   // ★모듈/캐릭터 표시규칙(항상 적용 = 리스처럼 렌더, [hsPortrait] 등). 리더 base render에서 expandCardRegex로.
  const created: any[] = [];
  for (let i = 0; i < eps.length; i++) {
    ...
    if (displayRulesArr) rec.displayRules = displayRulesArr;   // ★모듈/캐릭터 표시규칙(항상 적용 — 리더 base render).
...
function inboxParsed(it: any): { parsed: any; total: number } {
  ...
  const displayRules = (it && Array.isArray(it.displayRules)) ? it.displayRules : null;   // ★모듈/캐릭터 표시규칙(항상 적용)
  return { parsed: { char: name, fp: String((it && it.fp) || ''), assets, cleanupRegex, displayRules, chats: [{ name: '리스', messages }] }, total: messages.length };
}
